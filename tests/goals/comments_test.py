...
def test_create(auth_client, new_user, goal):
    response = auth_client.post(reverse("comment_create"),
...
def test_retrieve(auth_client, new_user, comment, goal):
    response = auth_client.get(reverse("comment_pk", args=[comment.pk]))
...
def test_update(auth_client, new_user, comment):
    response = auth_client.patch(reverse("comment_pk", args=[comment.pk]),
...
def test_delete(auth_client, new_user, comment):
    response = auth_client.delete(reverse("comment_pk", args=[comment.pk]))
