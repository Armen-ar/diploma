...
class TgBot:
    ...
    def get_goals_user(self, user_tg: TgUser) -> None:
        ...
        goals = (Goal.objects.filter(
            category__board__participants__user=user_tg.user
    ...
    def check_user(self, user_ud: int, chat_id: int) -> TgUser | bool:
        ...
        Если пользователя нет в базе, то создание записи в TgUser и ожидание
        подтверждение верификационного кода на сайте. Если TgUser найден, но не
        закреплен за User, то создание другой код верификации и его подтверждение.
        """
        user_tg, created = TgUser.objects.get_or_create(
            user_ud=user_ud,
            chat_id=chat_id
        )
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            for item in response.result:
                ...
                user_tg: TgUser | False = self.check_user(
                    user_ud=item.message.from_.id,
